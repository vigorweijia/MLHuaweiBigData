...
train_gps_path = '6_19_new_train_data.csv'
test_data_path = 'event_port/A_testData0531.csv'
order_data_path = 'event_port/loadingOrderEvent.csv'
port_data_path = 'event_port/port_archived1.csv'
...
def get_feature_test(df,port_data_path,train,train_data):
    ...
    temp_df['arrival'] = temp_df.apply(lambda x: x['TRANSPORT_TRACE'][x['TRANSPORT_TRACE'].rfind('-') + 1:],
                                       axis=1)  # 测试集的终点
    temp1 = train_data.groupby('loadingOrder', as_index=False).first()  # 找出训练集中每一个订单的任意一条记录，
    ...
    temp1['departure'] = temp1.apply(lambda x: dict_ports[x['departure']], axis=1)#训练集
    temp1['arrival'] = temp1.apply(lambda x: dict_ports[x['arrival']], axis=1)  # 分别换成真实的港口名
    temp_df = temp_df[['loadingOrder', 'departure', 'arrival']]
    #temp_df=temp_df.merge(dis,on='loadingOrder')#把直线距离也拼接上
    ...
    temp1=temp1.merge(train,on='loadingOrder')#merge的目的是为了提取出train的距离
    temp1=temp1[['loadingOrder', 'departure', 'arrival','distance']]#把训练集的距离提取出来
    #temp1.columns=['train_loadingOrder','departure','arrival','distance']#提取出每个训练订单的距离。
    #遍历temp_df中的每一行
    temp1=temp1.groupby(['departure','arrival'])['distance'].agg('mean').reset_index()
    temp1.columns=['departure','arrival','mean']
    dict_temp={}
    for index,row in temp1.iterrows():
        dict_temp[(row['departure'],row['arrival'])]=row['mean']
    temp_df=temp_df.merge(dis,on='loadingOrder')
    temp_df['distance']=temp_df.apply(lambda x:dict_temp[(x['departure'],x['arrival'])] if (x['departure'],x['arrival']) in dict_temp else temp_df['linear_distance'],axis=1)
...
def main():
    ...
    print(test.columns)
    print(train.shape)
    features = [c for c in train.columns if c not in ['count', 'label', 'loadingOrder','anchor_0_6','anchor_ratio_0_6','anchor_7_15','anchor_ratio_7_15','speed_mse','linear_distance']]
    ...
    result.to_csv('result-061911.csv')
    ...
    result = test_data[
        ['loadingOrder', 'timestamp', 'longitude', 'latitude', 'carrierName', 'vesselMMSI', 'onboardDate', 'ETA',
         'creatDate']]
    result.to_csv('testout-061911.csv', index=False)
