...
def get_feature_test(df,port_data_path,train,train_data):
    ...
    temp_df['distance']=temp_df.apply(lambda x:dict_temp[(x['departure'],x['arrival'])] if (x['departure'],x['arrival']) in dict_temp else x['linear_distance'],axis=1)
...
def main():
    ...
    result = test_data[
        ['loadingOrder', 'timestamp', 'longitude', 'latitude', 'carrierName', 'vesselMMSI', 'onboardDate', 'ETA',
         'creatDate']]
    result.to_csv('testout-0619_last.csv', index=False)
