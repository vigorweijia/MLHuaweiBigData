...
def get_feature_test(df,port_data_path,train,train_data):
    ...
    temp_df['distance']=temp_df.apply(lambda x:match_order(x,temp1),axis=1)
...
def build_model(train, test, pred, label, seed=1080, is_shuffle=True):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(kf_way, start=1):
        ...
        #train_pred[valid_idx] = mlp.predict(valid_x)
        #test_pred += mlp.predict(test[pred]) / fold.n_splits
...
def main():
    ...
    print(test.columns)
    print(train.shape)
    features = [c for c in train.columns if c not in ['count', 'label', 'loadingOrder','anchor_7_15','anchor_ratio_7_15','speed_mse','linear_distance', 'anchor_0_6', 'anchor_ratio_0_6']]
    ...
    result.to_csv('result-061901.csv')
    ...
    test_data['ETA'] = test_data['ETA'] + datetime.timedelta(days=7)
    ...
    result = test_data[
        ['loadingOrder', 'timestamp', 'longitude', 'latitude', 'carrierName', 'vesselMMSI', 'onboardDate', 'ETA',
         'creatDate']]
    result.to_csv('testout-061901.csv', index=False)
